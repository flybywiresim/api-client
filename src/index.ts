...
function get<T>(url: URL, headers?: any): Promise<T> {
    return axios.get<T>(url.href, { headers })
        .then((res) => res.data);
}

function del(url: URL, headers?: any): Promise<void> {
    return axios.delete(url.href, { headers })
        .then((res) => res.data);
}

function post<T>(url: URL, body: any, headers?: any): Promise<T> {
    const headersToSend = {
...
function put<T>(url: URL, body: any, headers?: any): Promise<T> {
    const headersToSend = {
        'Content-Type': 'application/json',
        ...headers,
    };
